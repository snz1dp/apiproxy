...
## 简介
本文件为Models查询接口的详细API文档，涵盖以下内容：
- 完整的GET /v1/models和GET /v1/models/{model}端点规范
- 响应格式与可用模型列表说明
...
## 项目结构
Models查询接口位于OpenAI兼容代理服务的v1版本路由中，通过FastAPI框架提供REST API能力。核心文件组织如下：
...
- 访问密钥验证工具：src/apiproxy/openaiproxy/api/utils.py
...
## 核心组件
- Models端点控制器：提供可用模型列表查询与单个模型详情查询
- 节点代理服务：负责从数据库加载节点与模型配置，维护节点健康状态，实现动态模型发现
- 数据模型：定义ModelCard、ModelList、ModelPermission等响应结构
- 访问密钥验证：确保API调用的安全性与权限控制
...
## 架构概览
Models查询接口的调用流程如下：
1. 客户端发起HTTP请求至/v1/models或/v1/models/{model}
2. FastAPI路由解析请求并调用对应的处理器函数
3. 处理器函数通过依赖注入获取NodeProxyService实例
4. NodeProxyService从数据库加载节点与模型配置，构建模型卡片列表
5. 返回ModelList或单个模型详情给客户端
...
Client->>API : GET /v1/models 或 GET /v1/models/{model}
API->>Handler : 调用处理器函数
Handler->>Service : 获取模型列表/详情
Service->>DB : 查询节点与模型配置
DB-->>Service : 返回节点与模型数据
Service-->>Handler : 返回模型卡片列表/详情
Handler-->>Client : 返回ModelList/模型详情
```
...
- 功能：返回当前可用的模型列表
- 认证：需要有效的API密钥
- 响应：ModelList对象，包含多个ModelCard条目
- 错误处理：无效密钥返回401；内部错误返回500

响应结构说明：
- object: 固定为"list"
- data: ModelCard数组，每个元素代表一个可用模型
...
- 认证：需要有效的API密钥
- 响应：单个ModelCard对象
- 错误处理：模型不存在返回404；无效密钥返回401；内部错误返回500

注意：当前实现中，该端点的路由定义存在于v1路由聚合中，但具体处理器函数需在models.py中实现。
