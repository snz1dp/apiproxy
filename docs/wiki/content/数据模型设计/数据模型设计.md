...
- 节点请求日志（ProxyNodeStatusLog）
  - 主键：id（UUID）
  - 外键：node_id → Node.id
  - 关键字段：node_id、proxy_id、status_id、ownerapp_id、action（枚举：completions/responses/audio_speech/audio_transcriptions/audio_translations/embeddings/images_edits/images_generations/images_variations/videos_generations/videos_retrieve/videos_cancel/videos_content/healthcheck/rerankdocs）、model_name、start_at、end_at、first_response_at、latency、stream、request_data、request_tokens、response_data、response_tokens、total_tokens、error、abort、error_message、error_stack、process_id、client_ip
