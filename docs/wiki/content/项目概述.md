...
## 引言
本项目是一个面向大模型服务的接口代理系统，旨在为上层应用提供统一、可扩展、可观测、可治理的 OpenAI 与 Anthropic 双协议接口能力。项目通过 FastAPI 构建高性能 Web 服务，结合多节点负载均衡、协议识别与转换、配额管理、请求日志与用量统计、健康检查与文档化接口等能力，帮助企业在复杂的大模型生态中实现安全、可控、高可用的服务代理。

项目定位与价值：
- 统一入口：以统一 `/v1` 前缀对外暴露 Chat Completions、Completions、Responses、Audio、Embeddings、Rerank 与 Anthropic Messages 等兼容接口，降低迁移成本与集成复杂度。
- 多节点代理：支持多后端节点接入与动态调度，提升吞吐与可用性。
- OpenAI 新接口兼容：Chat Completions、Completions 与 Responses 请求 schema 会保留 provider-specific 扩展字段，可透传 Omni 音频等新增参数。
...
- api：API 层，按版本划分路由，封装鉴权、协议识别、请求处理与响应。
- services/database：数据库模型与 CRUD，支撑配额、节点、应用、代理状态等数据。
- services/nodeproxy：节点代理核心逻辑，负责负载均衡、协议选择、配额校验、用量归集与错误处理。
...
RESP["api/v1/responses.py<br/>Responses 兼容实现"]
AUDIO["api/v1/audio.py<br/>音频接口实现"]
ANTH["api/v1/anthropic.py<br/>Anthropic 兼容接口实现"]
ADAPTER["api/v1/protocol_adapters.py<br/>协议适配层"]
end
subgraph "服务层"
...
- 路由与鉴权
  - v1 路由聚合 Chat Completions、Completions、Embeddings、Rerank、Models，以及 Anthropic Messages、Count Tokens、Message Batches 等接口。
  - 鉴权策略：管理接口使用管理密钥；北向推理接口统一使用应用 API Key，并兼容 `x-api-key` 的 Anthropic 风格访问。
- 协议适配层
  - 当北向请求协议与后端节点协议不一致时，负责在 OpenAI 与 Anthropic 之间进行请求体、响应体和流式事件转换。
- 节点代理服务
...
## 架构总览
下图展示了从客户端请求到后端节点的完整链路，以及协议识别、转换、配额与用量统计的关键环节。
