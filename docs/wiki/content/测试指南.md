...
### Responses 运行时代理测试
- 测试目标
  - 验证 `/v1/responses` 的扩展字段透传、非流式 usage 归集、流式 typed SSE 事件保真，以及节点模型配额异常映射。
- 关键点
  - 通过运行时依赖覆盖替换节点代理服务，直接验证路由层行为。
  - 同时覆盖 `NodeProxyService.stream_generate` 的 SSE 事件分块行为，确保 `event` 与 `data` 不会被错误拆散。
- 用例示例路径
  - [test_responses.py](file://src/apiproxy/tests/api/test_responses.py)
  - [test_nodeproxy_service.py](file://src/apiproxy/tests/services/test_nodeproxy_service.py)
...
### 节点代理服务测试
- 测试目标
  - 验证服务层在请求前后的配额处理、流式与非流式生成、超时与连接失败的错误映射、取消传播、SSE 事件分块与日志落盘。
- 关键点
