...
- 关键配置项与默认值（节选）：
  ...
  - proxy_request_timeout: int，默认 600，可通过 APIPROXY_PROXY_REQUEST_TIMEOUT 配置代理非流式请求超时。
  - proxy_stream_connect_timeout: int，默认 60；proxy_stream_read_timeout: int，默认 1800，可通过 APIPROXY_PROXY_STREAM_CONNECT_TIMEOUT 和 APIPROXY_PROXY_STREAM_READ_TIMEOUT 配置流式请求连接/读取超时。
